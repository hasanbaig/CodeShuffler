def similarity(a, b):
    #Helper function to calculate similarity between two given options
    a_parts = a.split(',')
    b_parts = b.split(',')
    matches = 0
...
def find_closest(options, correct_answer):
    #Find the most similar MCQ option to the correct answer
    best = None
    best_score = -1
...
def sort_by_similarity(options, correct_answer):
    #Sort the MCQ options based on their similarity to the correct answer
